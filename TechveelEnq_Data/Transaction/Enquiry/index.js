...
const InsertEnquiry = async (Enquirydata) => {
    try {
        ...
        .input('HscPer', sql.VarChar(50),Enquirydata.HscPer)
        ...
        .input('ReferenceContactNumber',sql.VarChar(30), Enquirydata.ReferenceContactNumber)
    ...
    try
     {
        let pool = await sql.connect(config.sql);
        const sqlQueries = await utils.loadSqlQueries('Transaction/Enquiry');
        const GetEnquirys = await pool.request()
...
const GetOneEnquiry = async (EnquiryId) => {
    try {
        let pool = await sql.connect(config.sql);
        const sqlQueries = await utils.loadSqlQueries('Transaction/Enquiry');
        const GetoneEnquiry = await pool.request()
